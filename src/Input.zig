...
pub const Input = struct {
    ...
    pub fn init(allocator: *std.mem.Allocator, display: *const c.Display, rootwindow: c.Window) !Input {
        ...
        _ = c.XGrabKey(@constCast(input.x_display), c.XKeysymToKeycode(@constCast(input.x_display), Config.add_focused_master_key), Config.add_focused_master_super, input.x_rootwindow, 0, c.GrabModeAsync, c.GrabModeAsync);
        _ = c.XGrabKey(@constCast(input.x_display), c.XKeysymToKeycode(@constCast(input.x_display), Config.add_focused_slave_key), Config.add_focused_slave_super, input.x_rootwindow, 0, c.GrabModeAsync, c.GrabModeAsync);
